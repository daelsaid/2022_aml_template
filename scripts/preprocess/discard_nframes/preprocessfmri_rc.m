function preprocessfmri_rc(SubjectI, ConfigFile)
...
%% load required toolbox for discarding volumes. (load nii.gz file)
basedir='/oak/stanford/groups/menon';
NIFTI_PATH = fullfile(basedir,'projects/zygao/misc/NIFTI');
addpath(NIFTI_PATH);
...
discard_num        = double(config.discard_num);
...
pipelinefamily = {'swar', 'swavr', 'swgcar', 'swgcavr', ...
    ...
    'swaor', 'swgcaor', 'swfaor', 'swgcfaor','swcfaor','swcr','sw','swgcr'};
    ...
    for irun = 1:numrun
        ...
        if isempty(inputimgprefix)
            ...
            unix(sprintf('cp -aLf %s %s', fullfile(unnorm_dir, ['I*', data_type, '*']), ...
                temp_dir));
%% discard volumes
            origin_s=load_nii_zip(sprintf('%s/I.nii.gz',temp_dir));

            tmp_data = origin_s.img;
            tmp_data(:,:,:,1:discard_num)=[];

            [~,~,~,num_trs]=size(tmp_data);

            tmp_filename = sprintf('%s/I.nii',temp_dir);
            origin_s.img = tmp_data;
            origin_s.hdr.dime.dim(5)=num_trs;

            save_untouch_nii(origin_s, tmp_filename);
            system(sprintf('gzip -f %s',tmp_filename));
        ...
        for cnt = 1:nstep
            ...
            switch p
                ...
                case 'w'
                    ...
                    preprocessfmri_normalizeD(wholepipeline, currentdir, template_path, boundingboxdim, [pipeline, inputimgprefix], inputimg_file, meanimg_file, temp_dir, SPGRfile_file, spm_path);
