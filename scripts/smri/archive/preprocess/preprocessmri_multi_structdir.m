...
% de 10/07/2022 added functionality that reads a file list of structural folders that deviate rom lab standard
    % example: /oak/stanford/groups/menon/rawdata/scsnl/10332/visit1/session1/anatomical/ORIG 3D_Volume3'
  ...
  %  ConfigFile = ConfigFile(1:end-2);
    ...
    spgr_dirname_list  = strtrim(config.spgr_dirname);
...
%==========================================================================
    ...
    SPGR_dir= ReadList(spgr_dirname_list);
    SPGR_folder        = SPGR_dir{subject_i};
    % (spgr_dirname_list);
% 
    %%%%% ----- check anatomical input file and image --------------
    spgr_dir= fullfile(data_dir, subject, ['visit',visit],['session',session],'anatomical', SPGR_folder);
